...
3. **Time Bucket Partitioning:** Instead of `SELECT * WHERE scheduled_at <= now()`, why partition jobs by time bucket (e.g., hour)? What happens to query performance at 1M+ jobs without partitioning? This design help avoid full table scan when the number of jobs is large, we can implement indexing and partition on time_bucket column, which can be parsed from scheduled_at column.
